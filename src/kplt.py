...
def left_ideal(gens, O):
    ...
    Z, d = quaternion_algebra_cython.integral_matrix_and_denom_from_rational_quaternions(all_gens)
    H = Z.hermite_form(include_zero_rows=False)
    basis = quaternion_algebra_cython.rational_quaternions_from_integral_matrix_and_denom(B, H, d)
...
def random_combination(basis, bound=1000):
    ...
    return sum(
            ZZ.random_element(-bound, bound + 1) * x_i for x_i in basis)
...
def find_generators(I):
    ...
    N = I.norm()
    alpha = I.basis()[0]
    while  alpha.reduced_norm() == 0 or Integer(alpha.reduced_norm()).divides(N**2):
        alpha = random_combination(I.basis())

    J = left_ideal([N, alpha], O)
    if not J == I:
        raise RuntimeError(
            'Oh dear. The algorithm failed.')
