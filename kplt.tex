...
So far we have reduced the problem of finding a left \( O \)-ideal with \( \ell \)-power norm equivalent to \( I \) to the same problem but we may assume that \( I \) has prime norm.
We will further assume that \( N \) is large, coprime to \( \ell \), 4 and \( p \).

We will now reduce the problem of
